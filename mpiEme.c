...
void p1(int N, int cp, int id)
{
    ...
    //vLoc = (int *)malloc(tamanio * sizeof(int));
    int *vLoc2 = (int *)malloc(tamanio * sizeof(int));
    MPI_Scatter(Vtot, tamanio, MPI_INT, vLoc2, tamanio, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    for (paso = 1; paso < cp; paso *= 2)
    {
        ...
        {
            // Los que envian al vecino
            MPI_Send(vLoc2, tamanioActual, MPI_INT, (id - paso), id, MPI_COMM_WORLD);
            // Espero que reciba
            free(vLoc2);
            break;
            // break
